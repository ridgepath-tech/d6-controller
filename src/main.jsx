...
function formatEvent(event) {
  ...
  if (event.type === "power") return event.state === "resting" ? "D6 display asleep with Windows" : "D6 display restored with Windows";
...
function App() {
  ...
  function hasKeySettings(definition) { return Boolean(definition && typeof definition === "object" && Object.keys(definition).length); }

  async function moveKey(sourceKey, targetKey, overwrite = false) {
    if (!profile || !profileName || sourceKey === targetKey) return;
    const sourceDefinition = keyDefinition(profile, scene, page, sourceKey);
    if (!hasKeySettings(sourceDefinition)) return;
    const targetDefinition = keyDefinition(profile, scene, page, targetKey);
    if (!overwrite && hasKeySettings(targetDefinition)) {
      setModal({ mode: "move", sourceKey, targetKey, error: "" });
      return;
    }
    ...
    selected.keys[String(targetKey)] = selected.keys[String(sourceKey)];
    delete selected.keys[String(sourceKey)];
    try {
      const response = await api(`/api/profiles/${encode(profileName)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(nextProfile) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Could not move key settings");
      setProfile(result); setSelectedKey(targetKey); setModal(null);
      notify(`Moved Key ${String(sourceKey).padStart(2, "0")} to Key ${String(targetKey).padStart(2, "0")}`);
    } catch (error) {
      if (modal?.mode === "move") updateModal("error", error.message); else notify(error.message);
    }
  }
  ...
  function beginPointerDrag(event, key) {
    if (!hasKeySettings(keyDefinition(profile, scene, page, key))) return;
    pointerDragRef.current = { sourceKey: key, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, moved: false };
    event.currentTarget.setPointerCapture?.(event.pointerId);
  }
  function movePointerDrag(event) {
    const pointerDrag = pointerDragRef.current;
    if (!pointerDrag) return;
    if (!pointerDrag.moved && Math.hypot(event.clientX - pointerDrag.startX, event.clientY - pointerDrag.startY) < 8) return;
    pointerDrag.moved = true; dragMovedRef.current = true; setDraggedKey(pointerDrag.sourceKey);
    const target = document.elementFromPoint(event.clientX, event.clientY)?.closest("[data-deck-key]");
    const targetKey = target ? Number.parseInt(target.getAttribute("data-deck-key"), 10) : null;
    setDragOverKey(Number.isNaN(targetKey) ? null : targetKey);
  }
  function finishPointerDrag(event) {
    ...
    if (!Number.isNaN(targetKey) && targetKey !== null && targetKey !== pointerDrag.sourceKey) moveKey(pointerDrag.sourceKey, targetKey);
  }
  ...
  async function saveAction() {
    ...
    try {
      ...
      setProfile(result); setModal(null); notify(`Action saved for key ${String(modal.key).padStart(2, "0")}`);
    } catch (error) { updateModal("error", error.message); }
  }
  ...
  return <div className="app-shell">
    ...
    <div className="layout"><aside className="sidebar">
      ...
      <section className="deck-frame" aria-label="D6 button layout"><div className="deck-grid">{KEY_LAYOUT.map((key) => { const definition = keyDefinition(profile, scene, page, key); const actionLabel = actionDisplayLabel(definition); const image = imageForKey(key); const configured = hasKeySettings(definition); return <button className={`deck-key ${selectedKey === key ? "selected" : ""} ${image ? "has-image" : "empty"} ${configured ? "draggable" : ""} ${draggedKey === key ? "drag-source" : ""} ${dragOverKey === key ? "drag-target" : ""}`} key={key} data-deck-key={key} onClick={() => clickKey(key)} onPointerDown={(event) => beginPointerDrag(event, key)} onPointerMove={movePointerDrag} onPointerUp={finishPointerDrag} onPointerCancel={finishPointerDrag} aria-label={`Configure Key ${key}`} title={configured ? `Drag Key ${String(key).padStart(2, "0")} to move its settings` : `Configure Key ${String(key).padStart(2, "0")}`}>{image ? <img src={image} alt="" draggable="false" /> : <span className="empty-art"><Icon name="plus" size={22} /><span>Configure</span></span>}<span className="key-overlay"><strong>{String(key).padStart(2, "0")}</strong><span>{actionLabel}</span></span></button>; })}</div></section>
