...
function App() {
  ...
  async function saveAction() {
    ...
    const resetFolderArtworkForLabel = modal.actionType === "open_folder" && Boolean(modal.label.trim()) && Boolean(existing.image);
    ...
    try {
      ...
      if (resetFolderArtworkForLabel) {
        const imageResponse = await api(`/api/profiles/${encode(profileName)}/scenes/${encode(scene)}/pages/${encode(page)}/keys/${modal.key}/image`, { method: "DELETE" });
        const imageResult = await imageResponse.json();
        if (!imageResponse.ok) throw new Error(imageResult.error || "Could not reset folder artwork");
        result = imageResult.profile;
      }
